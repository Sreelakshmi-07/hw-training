...
class AmazonScrape(scrapy.Spider):
	...
	def parse(self, response):
		...
		if reviews_link is None:
			...
			yield response.follow(url=reviews_link, headers=self.headers, callback=self.customer_reviews,cb_kwargs = {'url' : reviews_link})
	...
	def customer_reviews(self, response,url):

		date_list = []
		review_list =[]
		review_author = response.xpath(
			'//div[@id="cm_cr-review_list"]//div[@class="a-profile-content"]/span/text()').extract()
		review_date = response.xpath(
			'//div[@id="cm_cr-review_list"]//span[@data-hook="review-date"]/text()').extract()
		...
		review_rating = response.xpath(
			'//div[@id="cm_cr-review_list"]//i[@data-hook="review-star-rating"]/span/text()').extract()
		review_rating_formatted = [float(i[0:3]) for i in review_rating]
		review_text = response.xpath(
			'//div[@id="cm_cr-review_list"]//span[@data-hook="review-body"]/span/text()').extract()

		for i,j,k,l in zip(review_author,date_list,review_rating_formatted,review_text):
			reviews_dict = {'review_author' : i,'review_date':j,'review_rating_formatted':k,'review_text':l}
		...
		# next_review_page = response.xpath(
		# 	'//ul[@class="a-pagination"]/li[@class="a-last"]/a/@href').extract_first()
		...
		# yield scrapy.Request(url=next_review_page, headers=self.headers, callback=self.customer_reviews)

		# next_review_page_next = re.sub("pageNumber=2","pageNumber=3",next_review_page)
		# yield scrapy.Request(url=next_review_page_next, headers=self.headers, callback=self.parse)

	def ajax_requests(self, response, product_url, product_asin, product_name, brand, star_rating, price, old_price, size, color, date_first_available, seller,link):
		...
		# review_author = response.meta.get('review_author')
		yield {

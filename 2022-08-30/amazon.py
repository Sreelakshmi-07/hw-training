...
class AmazonScrape(scrapy.Spider):
	...
	def ajax_requests(self, response, proDetails):
		...
		# s = self.customer_reviews(response)
		...
		if reviews_link is None:
			...
			page = 1
			yield scrapy.Request(url=reviews_link+str(page), headers=self.headers, callback=self.customer_reviews,cb_kwargs = {'proDetails':item})

	def customer_reviews(self, response,proDetails):
		...
		for i, j, k, l in zip(review_author, date_list, review_rating_formatted, review_text):
			reviews_dict = {'review_author': i, 'review_date': j,
							'review_rating_formatted': k, 'review_text': l}
		...
		yield {
			...
			'date_first_available':  proDetails['date_first_available'],
			'seller': proDetails['seller'],
			'offer_listing': proDetails['offer_listing'],
			'reviews' : review_list
		}
		next_page = response.xpath(
		...
		if next_page:
			...
			for page in range(2, 4):
				next_page_url = re.sub(
					"pageNumber=2", "pageNumber="+str(page), next_page)
				if page <= 2:
					yield response.follow(url = next_page_url,headers=self.headers,callback = self.customer_reviews)
				else:
					yield response.follow(url = next_page_url,headers=self.headers,callback = self.customer_reviews)

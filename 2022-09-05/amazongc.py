...
class AmazonScrape(scrapy.Spider):
	...
	def start_requests(self):
		...
		yield scrapy.Request(url=url, headers=self.headers, callback=self.link_response
								 )
	...
	def link_response(self, response):
		...
		if next_page:
			...
			yield response.follow(url = next_page,callback=self.link_response)
	...
	def parse(self, response):
		...
		star_ratings = response.xpath(
			'//div[@id="averageCustomerReviews"]//span[@class="a-icon-alt"][text()[contains(.,"out of 5 stars")]]/text()').extract_first()
	...
	def parse_customer_reviews(self, response, proDetail):
		...
		if no_reviews:
			...
			reviews_list = response.xpath('//div[@id="cm_cr-review_list"]//div[@data-hook="review"]')
		...
		next_page = response.xpath('//div[@data-hook="pagination-bar"]//a/@href').extract_first()
		if next_page:
			...
			for pages in range(2,4):
				...
				yield response.follow(url = next_page_urls,headers=self.headers,callback = self.parse_customer_reviews , cb_kwargs = {'proDetail':proDetail})

...
class AmazonScrape(scrapy.Spider):
	...
	'https://www.amazon.in/Gaming-Racing-GT-700-Ergonomic-Leather/dp/B09TB3TJX7/ref=sr_1_1?keywords=gaming+chairs&qid=1662061708&s=home-furnishings&sr=1-1']
	headers = {
	...
	def parse(self, response):
		...
		# self.result = []9
			...
			yield scrapy.Request(url=np, headers=self.headers, callback=self.customer_reviews,cb_kwargs = {'proDetail' : proDetails})


	def ajax_requests(self, response, proDetail):
	...
	def customer_reviews(self, response,proDetail):
		...
		for review in reviews:
			data={'r': review.xpath(
					'//div[@class="a-profile-content"]/span/text()').extract()}
		#  = review
		proDetail['link'].append(data)
		c = proDetail['link'] if reviews else 'n'
		proDetails = {
			'product_url': proDetail['product_url'],
			'product_asin': proDetail['product_asin'],
			'product_name': proDetail['product_name'],
		...
		yield scrapy.Request(url=url_ajax, headers=self.headers, callback=self.ajax_requests,
							 cb_kwargs={'proDetail': proDetails}
							 )
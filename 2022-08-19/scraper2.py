...
class AmazonScrape(scrapy.Spider):
	...
	def parse(self, response):
		...
		brand = response.xpath("//table[@id='productDetails_detailBullets_sections1']/tr/th[text()[contains(.,'Brand')]]/following-sibling::td/text()").extract_first() or response.xpath("//table[@class='a-normal a-spacing-micro']/tr[@class='a-spacing-small po-brand']/td[@class='a-span9']/span/text()").extract_first()
		product_brand = 'N/A' if brand is None else brand.encode("ascii", "ignore").decode().strip()
		star_ratings = response.xpath(
			'//span[@class="a-icon-alt"][text()[contains(.,"out of 5 stars")]]/text()').extract_first()
		product_star_ratings = 0 if star_ratings is None else float(star_ratings[0:3])
		...
		product_size = 'N/A' if size is None else size.encode("ascii", "ignore").decode().strip().replace('\"',"")
		...
		product_color = 'N/A' if color is None else  color.encode("ascii", "ignore").decode().strip()
		...
		date_formatted = 'N/A' if date is None else date.encode("ascii", "ignore").decode().strip().replace(',','')
		product_date = datetime.datetime.strptime(date_formatted, '%B %d %Y').date()

...
class HuffReality(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for agents in response.xpath('//div[@id="rnRoster"]/article'):
            ...
            profile_url = agents.xpath(
                './/div[@class="rn-agent-icon-website"]/a/@href').extract_first(default='N/A')
            ...
            website = agents.xpath(
                './/div[@class="rn-agent-icon-website"]/a/@href').extract_first(default='N/A')
            website_error = agents.xpath(
               './/div[@class="rn-agent-icon-website"]/a[contains(@href,"http://http")]/@href').extract_first()
            image_url = agents.xpath(
                './/div[@class="rn-agent-photo-languages"]/img/@src').extract_first(default='N/A')
            agent_phone_numbers = agents.xpath(
                './/span[@class="rn-agent-contact-main-phone-number"]/text()').extract_first(default='N/A')
            office_phone_numbers = agents.xpath(
                './/span[@class="rn-agent-contact-office-phone-number"]/text()').extract_first(default='N/A')
    ...
    def parse_details(self, response):
        ...
        email = response.xpath(
               '//div[@class="site-home-page-content-text"]/p[text()[contains(.,"@")]]/text()'
             ).extract_first()
        emails = 'N/A' if email is None else email.split("email")[-1]
